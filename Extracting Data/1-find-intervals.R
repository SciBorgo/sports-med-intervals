...
# Set up text extraction functions and conduct search of abstracts
...
### Section 3: find abstracts and extract CIs
# Change line 286 if extraction in batches

# now loop one article at a time and extract data from abstract
data = any.data = NULL # 
for (a in 1:nrow(meta)){ # can do shorter runs than nrow(meta) if online connection to pubmed breaks
...
#    if(any(is.na(ci.numbers))==TRUE){cat('Empty CI level for', rec$pmid, ', strings=', str_sub(abstract, ci.places), '\n')}
  ...
  if(a%%100==0){
    cat('Up to ', a, '.\n', sep='') ## update progress
    Sys.sleep(60) # break for 10 mins (try to avoid over-loading pubmed)
    } 
} # end of any abstract data

# Save data using appropriate file name
save(data, any.data, a, file = 'intervals-first-batch-sample1.RData')
