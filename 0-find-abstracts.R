...
numbers = get_abs(journals = journals)


# Plot papers each year
p_paper_year = ggplot(data=numbers, aes(x=year, y=count, col=factor(journal)))+
  geom_point()+
  geom_line()+
  #scale_color_manual('Journal', values=cbPalette)+ # cbPalette is missing
